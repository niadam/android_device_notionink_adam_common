...
/*        if (flags) {
        ...
        err = err<0 ? -errno : 0;
        AALOGE_IF(err, "KXTF9_IOCTL_SET_ENABLE failed (%s)", strerror(-err));
        if (!err) {
            mEnabled = flags;
        }
        if (!flags) {
...
int AccelerationSensor::readEvents(sensors_event_t* data, int count)
{
    ...
    while (count && mInputReader.readEvent(&event)) {
        int type = event->type;
//        ALOGW("AccelerationSensor (type=%d, code=%d)", type, event->code);
...
//    ALOGW("AccelerationSensor numEventReceived=%d", numEventReceived);
